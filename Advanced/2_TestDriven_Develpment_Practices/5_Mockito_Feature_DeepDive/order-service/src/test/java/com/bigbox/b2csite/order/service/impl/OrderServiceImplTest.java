...
public class OrderServiceImplTest {
	...
	public void test_openNewOrder_successfullyRetriesDataInsert() throws Exception {
		...
		.thenThrow(new DataAccessException("Firts ex")).thenReturn(1);
	...
	public void test_openNewOrder_success() throws Exception {
		...
		OrderEntity captureOrderEntity = orderEntityCaptor.getValue();
		
		Assert.assertNotNull(captureOrderEntity);
		Assert.assertEquals(CUSTOMER_ID, captureOrderEntity.getCustomerId());
		Assert.assertEquals(orderNumber, captureOrderEntity.getOrderNumber());
	}
}
